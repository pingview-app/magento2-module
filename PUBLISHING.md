...
`github.com/pingview-app/magento2-module`, które nie istnieje.

## Kroki

| # | Co | Po co | Jak |
|---|----|-------|-----|
| 1 | Publiczne repo `pingview-app/magento2-module` | Packagist przyjmuje tylko publiczny URL; nazwa musi zgadzać się z `support.source` w `composer.json` | `gh repo create pingview-app/magento2-module --public --source=. --remote=origin --push` |
| 2 | Tag `1.2.0` | `composer.json` nie ma pola `version` — Packagist bierze wersję z taga. CI odrzuca tag ≠ `ApiClient::VERSION` | `git tag 1.2.0 && git push origin 1.2.0` |
...
| 4 | Submit na Packagist | Rejestracja pakietu | packagist.org → **Submit** → wklej `https://github.com/pingview-app/magento2-module` → **Check** → **Submit** |
...
2. `git tag X.Y.Z && git push origin X.Y.Z` — Packagist odbiera hook, koniec.
